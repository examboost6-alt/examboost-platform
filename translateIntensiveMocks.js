...
async function processMock(mockIndex) {
    const filePath = "d:/examboost/src/app/test/[testId]/data/neetIntensiveMock" + mockIndex + ".ts";
    if (!fs.existsSync(filePath)) {
        console.log("File " + filePath + " not found");
        return;
    }

    console.log("Processing NEET Intensive Mock " + mockIndex + "...");
    ...
    for (let i = 0; i < qList.length; i++) {
        ...
        if (i % 25 === 0 && i > 0) {
            console.log("Mock " + mockIndex + ": translated " + i + " questions so far...");
        }
    }

    if (modified) {
        const outStr = "import { QuestionType } from '../mockData';\n\nexport const neetIntensiveMock" + mockIndex + "Questions: QuestionType[] = " + JSON.stringify(qList, null, 2) + ";\n";
...
async function run() {
    ...
    console.log("All intensive mocks fully translated!");
}
