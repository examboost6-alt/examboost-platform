...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (updatePayloadStr) {
      updatePayload = JSON.parse(updatePayloadStr);
    }
    ...
    // Get signed URL for frontend state update if needed, but not strictly required
    const { data: publicUrlData } = supabaseAdmin.storage.from('student-photos').getPublicUrl(photoPath);

    return NextResponse.json({ success: true, photoPath, publicUrl: publicUrlData?.publicUrl });
  } catch (e: any) {
