...
export const mock12Questions: QuestionType[] = Array.from({ length: 75 }).map((_, i) => {
    ...
    if (subject === 'Physics') {
        const physQ: any = physicsQuestions_12[subjectIndex];
        return {
            id: i + 1,
            subject: subject,
            type: physQ.type as 'MCQ' | 'Numerical',
            textEn: physQ.textEn || physQ.text || `[Mock 12] Physics Question ${subjectIndex + 1}`,
            textHi: physQ.textHi || physQ.text || `[Mock 12] Physics Question ${subjectIndex + 1} (Hi)`,
            options: physQ.type === 'MCQ' && physQ.options ? physQ.options.map((opt: any, idx: number) => ({ 
                id: idx + 1, 
                textEn: typeof opt === 'string' ? opt : opt.en || `Mock 12 Option ${idx + 1}`, 
                textHi: typeof opt === 'string' ? opt : opt.hi || `Mock 12 Option ${idx + 1} (Hi)` 
            })) : undefined,
    ...
    if (subject === 'Chemistry') {
        const chemQ: any = chemistryQuestions_12[subjectIndex];
        return {
            id: i + 1,
            subject: subject,
            type: chemQ.type as 'MCQ' | 'Numerical',
            textEn: chemQ.textEn || chemQ.text || `[Mock 12] Chemistry Question ${subjectIndex + 1}`,
            textHi: chemQ.textHi || chemQ.text || `[Mock 12] Chemistry Question ${subjectIndex + 1} (Hi)`,
    ...
    if (subject === 'Mathematics') {
        const mathQ: any = mathQuestions_12[subjectIndex];
        return {
            id: i + 1,
            subject: subject,
            type: mathQ.type as 'MCQ' | 'Numerical',
            textEn: mathQ.textEn || mathQ.text || `[Mock 12] Math Question ${subjectIndex + 1}`,
            textHi: mathQ.textHi || mathQ.text || `[Mock 12] Math Question ${subjectIndex + 1} (Hi)`,
